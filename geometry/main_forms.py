
# Ромб
rhombus_area = """
              <label class="form-label" for="a">Введите сторону <b>(A)</b></label>
              <input type="number" class="form-control" id="a" name="a" step="any" required>
            
              <label class="form-label" for="h">Введите высоту <b>(H)</b></label>
              <input type="number" class="form-control" id="h" name="h" step="any" required>
              """

rhombus_perimeter = """
                    <label class="form-label" for="a">Введите сторону <b>(A)</b></label>
                    <input type="number" class="form-control" id="a" name="a" step="any" required>
                    """

# Круг
circle = """
        <label class="form-label" for="radius">Введите радиус окружности <b>(R)</b></label>
        <input type="number" class="form-control" id="radius" name="radius" step="any" required>
        """

# Куб
cube = """
        <label class="form-label" for="a">Введите ребро <b>(A)</b></label>
        <input type="number" class="form-control" id="a" name="a" step="any" required>
        """

# Прямоугольник
rectangle = """
            <label class="form-label" for="a">Введите сторону <b>(A)</b></label>
            <input type="number" class="form-control" id="a" name="a" step="any" required>

            <label class="form-label" for="a">Введите сторону <b>(B)</b></label>
            <input type="number" class="form-control" id="b" name="b" step="any" required>"""

# Квадрат
square = """
        <label class="form-label" for="a">Введите сторону <b>(A)</b></label>
        <input type="number" class="form-control" id="a" name="a" step="any" required>"""


trap_area = """
            <label class="form-label" for="a">Введите основание <b>(A)</b></label>
            <input type="number" class="form-control" id="a" name="a"step="any" required>

            <label class="form-label" for="b">Введите основание <b>(B)</b></label>
            <input type="number" class="form-control" id="b" name="b" step="any" required>

            <label class="form-label" for="h">Введите высоту <b>(H)</b></label>
            <input type="number" class="form-control" id="h" name="h" step="any" required>"""


trap_perimeter = """
                <label class="form-label" for="a">Введите основание <b>(A)</b></label>
                <input type="number" class="form-control" id="a" name="a" step="any" required>

                <label class="form-label" for="b">Введите основание <b>(B)</b></label>
                <input type="number" class="form-control" id="b" name="b" step="any" required>

                <label class="form-label" for="c">Введите сторону <b>(C)</b></label>
                <input type="number" class="form-control" id="c" name="c" step="any" required>

                <label class="form-label" for="d">Введите сторону <b>(D)</b></label>
                <input type="number" class="form-control" id="d" name="d" step="any" required>"""